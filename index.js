...
// Globals expostos pelo SillyTavern
const _es  = () => window.eventSource;
const _et  = () => window.event_types;
const _cfg = () => window.extension_settings;
const _ssd = () => window.saveSettingsDebounced;
const _ctx = () => (typeof getContext === 'function' ? getContext() : window.SillyTavern?.getContext?.());
const _sep = () => window.setExtensionPrompt;

function cfg() {
    if (!_cfg()[EXT]) _cfg()[EXT] = {};
    if (!_cfg()[EXT].config) _cfg()[EXT].config = {
        githubToken: '', gistId: '', deepseekApiKey: '', semanticEnabled: false,
    };
    return _cfg()[EXT].config;
}

function skills() {
    if (!_cfg()[EXT]) _cfg()[EXT] = {};
    if (!_cfg()[EXT].skills) _cfg()[EXT].skills = {};
    return _cfg()[EXT].skills;
}
...
function detectTriggers(chat) {
    ...
    if (changed) { _ssd()(); renderSkills(); }
}

// Interceptor de geração via eventSource (padrão real do ST)
async function onBeforeGenerate() {
    try {
        const txt = activeText();
        if (!txt) return;
        if (typeof _sep() === 'function') {
            _sep()('HANNACORE', txt, 1, 0);
        }
    } catch(e) {
...
function renderSkills() {
    ...
    for (const [name, s] of Object.entries(skills())) {
        ...
        else { btn.onclick = () => { s.on = !s.on; _ssd()(); renderSkills(); }; }
...
async function init() {
    ...
    _es().on(_et().MESSAGE_RECEIVED, (idx) => {
        try {
            const ctx = _ctx();
    ...
    _es().on(_et().GENERATE_BEFORE_COMPLETION, onBeforeGenerate);
    injectUI();
